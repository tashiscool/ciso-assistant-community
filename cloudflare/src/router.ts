...
import {
  handleCcmRoutes,
  handleCryptoRoutes,
  handleFedrampCommunicationRoutes,
  handleScopeRoutes,
  handleScnRoutes,
  handleSecureConfigRoutes,
  handleTrustCenterRoutes,
  handleVdrRoutes,
} from './services/fedramp/http';
  ...
  try {
    switch (service) {
      ...
      case 'grc':
        ...
        break;
      case 'trust-center':
        response = await handleTrustCenterRoutes(rest, baseCtx);
        break;
      case 'fedramp-communications':
        response = await handleFedrampCommunicationRoutes(rest, baseCtx);
        break;
      case 'vdr':
        response = await handleVdrRoutes(rest, baseCtx);
        break;
      case 'ccm':
        response = await handleCcmRoutes(rest, baseCtx);
        break;
      case 'scn':
        response = await handleScnRoutes(rest, baseCtx);
        break;
      case 'secure-config':
        response = await handleSecureConfigRoutes(rest, baseCtx);
        break;
      case 'scope':
        response = await handleScopeRoutes(rest, baseCtx);
        break;
      case 'crypto':
        response = await handleCryptoRoutes(rest, baseCtx);
        break;
      default:
