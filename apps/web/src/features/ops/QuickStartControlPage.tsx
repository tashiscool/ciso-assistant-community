...
export function QuickStartControlPage() {
  ...
  if (loading) {
    return <div className="panel p-6 text-sm text-slate-300">Loading guided setup...</div>;
  }

  if (error || !overview) {
    return <div className="notice-error">{error ?? 'Guided setup could not be loaded.'}</div>;
  }
    ...
    <div className="space-y-6">
      <section className="panel overflow-hidden">
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white">Guided Setup</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">
              Use this setup path to stand up domains, permissions, governance sources, and the first working flows so Regovise becomes the system of record from day one.
            </p>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="metric-card">
          ...
          <div className="mt-2 text-xs text-slate-500">The actions still needed before the full program flow feels ready.</div>
        </div>
        <div className="metric-card">
          ...
          <div className="mt-2 text-xs text-slate-500">Frameworks and control sources already available for the program.</div>
        </div>
        <div className="metric-card">
          <div className="metric-label">External workflows</div>
          <div className="metric-value">{overview.analytics.find((item) => item.id === 'portal')?.value ?? 0}</div>
          <div className="mt-2 text-xs text-slate-500">Portal and collaboration work already active for this tenant.</div>
        </div>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="panel-subtle">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">Setup path</h2>
              <p className="text-sm text-slate-400">The concrete sequence for moving a workspace from initial setup into real program operations.</p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="panel-subtle">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">Helpful next destinations</h2>
              <p className="text-sm text-slate-400">Start points that help a new tenant move from setup into real day-to-day use.</p>
            </div>
          </div>
            <div className="mt-5 space-y-3">
          ...
          <div className="panel-subtle">
            <h2 className="text-lg font-semibold text-white">Early operating signals</h2>
            <p className="mt-1 text-sm text-slate-400">These counts show whether the tenant is already moving into live program work.</p>
